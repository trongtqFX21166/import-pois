...
{
    public class WorkerReMappingPlaceGGRawDataService : BackgroundService
    {
        private readonly IGoogleRawRepo _googleRawRepo;
        private readonly IVMPoiRawRepository _vmPoiRawRepository;
        private readonly IServiceProvider _serviceProvider;

        private readonly ILogger<WorkerReMappingPlaceGGRawDataService> _logger;
        private readonly HttpClient _peakClient;
        private readonly IConsumer _consumer;

        public WorkerReMappingPlaceGGRawDataService(
            IGoogleRawRepo googleRawRepo
            , IVMPoiRawRepository vmPoiRawRepository
            , IServiceProvider serviceProvider
            , ILogger<WorkerReMappingPlaceGGRawDataService> logger
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            var total = await _googleRawRepo.GetTotalReGGMapping();
            ...
            for (int skip = 0; skip <= pageCount; skip++)
            {
                ...
                var _datas = await _googleRawRepo.GetReGGMappings(skip, pageSize);
                if (_datas.Any())
                {
                    var _vmIds = _datas.Select(x => x.VmId).ToList();
                    var _pois = await _poiRepository.GetPoiByVmId(_vmIds);

                    // update search text
                    foreach(var data in _datas)
                    {
                        var _poi = _pois.FirstOrDefault(x => x.Item1 == data.VmId);
                        var _searchText = $"{_poi.Item2.Name}, {_poi.Item2.Address}";
                        data.SearchText = _searchText;

                        await _googleRawRepo.UpsertMappingTracking(data);

                        // add search text
                        _allSearchText.Add(data.SearchText);
                    }
                }
                
            }
            _logger.LogInformation($"reupdate-search-text Done");
