...
{
    public class WorkerCrawlerWazePlaceService : BackgroundService
    {
        ...
        public WorkerCrawlerWazePlaceService(
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            //await manualImportRunId("noCBNo20B25fNVxHc", versionCode);

            //3. get total placeIds with new versionCode
            int pageSize = 500;
            //string run_id = "yEMl8of12fCQoeWSn";
            var total = await _googleRawRepo.GetTotalInvalidWazeMapping();
            ...
            for (int skip = 0; skip <= pageCount; skip++)
            {
                ...
                var _datas = await _googleRawRepo.GetInvalidWazeMappings(skip, pageSize);
                if (_datas.Any())
                {
                    //await UpdateMappingManual(_datas
                    //    .Where(x => !string.IsNullOrWhiteSpace(x.WazeAddress))
                    //    .Select(x => x.WazeAddress).ToList());

                    //List<string> _placeIds = _datas.Where(x => !string.IsNullOrWhiteSpace(x.WazeId)
                    //&& x.WazeId.StartsWith("googlePlaces.")).Select(x => x.WazeId).ToList();
                    //List<string> _alterIds = _datas.Where(x => !string.IsNullOrWhiteSpace(x.WazeId)
                    //&& x.WazeId.StartsWith("venues.") && !string.IsNullOrWhiteSpace(x.WazeAlterId))
                    //    .Select(x => x.WazeAlterId).ToList();
                    //_placeIds.AddRange(_alterIds);

                    //await UpdateMappingManualByPlaceIds(_datas.Select(x => x.WazeId).ToList());
            ...
            for (int skip = 0; skip <= pageCount; skip++)
            {
                ...
                var _crawlerSearchTexts = searchTexts.ToArray();
                RunInfo runInfo = await createTaskAndStartRun(_crawlerSearchTexts);
                if (runInfo == null
                ...
                try
                {
                    ...
                    if (runRsult?.run_status == "SUCCEEDED")
                    {
                        if (runRsult.data.Count > 0)
                        {
                            ...
                            var mappingPlaces = await _googleRawRepo.GetWazeMappingTrackings(searchTexts);
                            foreach (var mappingPlace in mappingPlaces)
                            {
                                var _rawGG = rawGGs.Where(x => x.searchString == mappingPlace.WazeAddress).FirstOrDefault();
                                ...
                                {
                                    var distance = DistanceCalculator.CalculateDistance(mappingPlace.WazeLatitude, mappingPlace.WazeLongitude
                                        , _rawGG.location.lat, _rawGG.location.lng) * 1000;

                                    mappingPlace.GgLat = _rawGG.location.lat;
                                    mappingPlace.GgLng = _rawGG.location.lng;
                                    mappingPlace.GgName = _rawGG.title ?? string.Empty;
                                    mappingPlace.GgAddrress = _rawGG.address ?? string.Empty;
                                    mappingPlace.GgPlaceId = $"googlePlaces.{_rawGG.placeId}";
                                    mappingPlace.GgDistance = distance;
                                    mappingPlace.GgIsValid = distance < 400;
                                    mappingPlace.GgCategoryName = _rawGG.categoryName ?? string.Empty;
                                }

                                await _googleRawRepo.UpsertWazeMappingTracking(mappingPlace);
                            }
                        }
        ...
        {
            List<string> _searchTexts = searchTexts.Select(x => x).ToList();

            var rawGGs = await _googleRawRepo.GetCrawlerGooglesBySearchText(_searchTexts.ToArray());
            if (rawGGs?.Any() ?? false)
            {
                var mappingPlaces = await _googleRawRepo.GetWazeMappingTrackings(searchTexts);
                ...
                foreach (var mappingPlace in mappingPlaces)
                {
                    var _rawGG = rawGGs.Where(x => x.searchString == mappingPlace.WazeAddress
                    || x.searchString == mappingPlace.WazeAddress.Replace(" ", "%20")).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        mappingPlace.GgIsValid = distance < 400;
                        mappingPlace.GgCategoryName = _rawGG.categoryName ?? string.Empty;
                        mappingPlace.IsIgnoreMapping = false;
                    }
        ...
        private async Task UpdateMappingManualByPlaceIds(List<string> ggPlaceIds)
        {
            List<string> _searchTexts = ggPlaceIds.Select(x => x.Replace("googlePlaces.", "")).ToList();

            var rawGGs = await _googleRawRepo.GetCrawlerGoogles(_searchTexts.ToArray());
            if (rawGGs?.Any() ?? false)
            {
                var mappingPlaces = await _googleRawRepo.GetWazeMappingTrackingsByPlaceIds(ggPlaceIds);

                foreach (var mappingPlace in mappingPlaces)
                {
                    var _rawGG = rawGGs.Where(x => x.placeId == mappingPlace.WazeId.Replace("googlePlaces.","")
                    || x.placeId == mappingPlace.WazeAlterId.Replace("googlePlaces.", "")).FirstOrDefault();
